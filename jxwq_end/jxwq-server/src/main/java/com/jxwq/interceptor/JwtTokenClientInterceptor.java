...
public class JwtTokenClientInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(@Nullable HttpServletRequest request, @Nullable HttpServletResponse response, @Nullable Object handler) throws Exception {
        ...
        // 校验令牌
        try {
            // log.info("客户端jwt校验:{}", token);
            Claims claims = JwtUtil.parseJWT(jwtProperties.getClientSecretKey(), token);
            // log.info("客户端jwt校验成功，解析出来的用户信息 - " + claims);
            CurrentUserInfo currentUserInfo = new CurrentUserInfo();
            Integer clientUserId = CommonUtils.string2Int(CommonUtils.stringValue(claims.get(JwtClaimsConstant.CLIENT_USER_ID)));
            currentUserInfo.setUserType(SystemConstant.CLIENT_USER_TYPE); // 客户端用户类型
            currentUserInfo.setId(clientUserId);
            currentUserInfo.setName(CommonUtils.stringValue(claims.get(JwtClaimsConstant.CLIENT_USER_NAME)));
