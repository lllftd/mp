...
const getLikeList = async () => {
  ...
  const newLike = tweets.map(tweet => ({
    ...tweet,
  ...
  likeList.value = [...likeList.value, ...newLike];
  console.log("最终 likeList", likeList.value);
};
...
<template>
  <div>
    <div class="container">
      <div class="content" v-if="likeList.length > 0">
        <div class="list">
          <div class="list-item" v-for="item in likeList" :key="item.id" @click="goDetail(item.tweetsId)">
      ...
      <div v-else class="nodata flex flex-column flex-x-center flex-y-center">
        ...
        <div class="text">您还没有点赞任何美食哦～</div>
      </div>
    </div>
  </div>
</template>
